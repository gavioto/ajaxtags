...
Object.extend(String.prototype, (function() {
  var scriptFragmentMatchAll = new RegExp(Prototype.ScriptFragment, 'img'),
    ...
    NATIVE_JSON_PARSE_SUPPORT = window.JSON &&
...
Element.Methods = {
  ...
  previousSiblings: function(element, maximumLength) {
    return Element.recursivelyCollect(element, 'previousSibling', maximumLength);
  },

  nextSiblings: function(element, maximumLength) {
    return Element.recursivelyCollect(element, 'nextSibling', maximumLength);
  },
  ...
  makeClipping: function(element) {
    element = $(element);
    if (Object.isString(element._overflow)) return element; // FIX
...
if (Prototype.Browser.Opera) {
  ...
  Element.Methods.getStyle = function(element, style) {
    ...
    if (value == 'auto') {
      if ((style == 'width' || style == 'height' || style=='left' || style=='top') && (element.getStyle('display') != 'none')) // FIX
...
(function() {
  ...
  function _createResponder(element, eventName, handler) {
    ...
    if (Object.isUndefined(registry)) {
      if (Prototype.Browser.IE) CACHE.push(element); // FIX
    ...
    if (Object.isUndefined(respondersForEvent)) {
      respondersForEvent = [];
      if (!registry.src && element.inspect) registry.src = element.inspect(); // debug
  ...
  function stopObserving(element, eventName, handler) {
    ...
    if (registry.size() === 0) { // FIX
      Element.getStorage(element).unset('prototype_event_registry');
      /*var uid = (element === window) ? 0 : element._prototypeUID;
      Element.Storage[uid].unset('prototype_event_registry');
      if (Element.Storage[uid].size() === 0) {
        delete Element.Storage[uid];
        element._prototypeUID = void 0;
      }*/
      if (Prototype.Browser.IE) {
        i = CACHE.indexOf(element);
        if (i >= 0) {
          CACHE[i] = null;
        }
      }
